...
ggplot(aes(fixed.acidity), data = data) +
  geom_histogram(bins = 60, fill = 'darkred') +
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$fixed.acidity)$out

ggplot(aes(fixed.acidity), 
       data = subset(data, !(fixed.acidity %in% outliers))) +
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$volatile.acidity)$out

ggplot(aes(volatile.acidity), 
       data = subset(data, !(volatile.acidity %in% outliers))) +
  geom_histogram(bins = 50, fill = 'darkred') +
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$citric.acid)$out

ggplot(aes(citric.acid), 
       data = subset(data, !(citric.acid %in% outliers))) +
  geom_histogram(bins = 50, fill = 'darkred') +
  scale_x_continuous(breaks = seq(0, 20, 0.05))
```
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$residual.sugar)$out

ggplot(aes(residual.sugar), 
       data = subset(data, !(residual.sugar %in% outliers))) +
  geom_histogram(bins = 25, fill = 'darkred') +
  scale_x_continuous(breaks = seq(0, 20, 0.25))
```
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$chlorides)$out

ggplot(aes(chlorides), 
       data = subset(data, !(chlorides %in% outliers))) +
  geom_histogram(bins = 25, fill = 'darkred') +
  scale_x_continuous(breaks = seq(0, 20, 0.005))
```
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$total.sulfur.dioxide)$out

ggplot(aes(total.sulfur.dioxide), 
       data = subset(data, !(total.sulfur.dioxide %in% outliers))) +
  geom_histogram(bins = 25, fill = 'darkred') +
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$density)$out

ggplot(aes(density), 
       data = subset(data, !(density %in% outliers))) +
  geom_histogram(bins = 50, fill = 'darkred') +
  scale_x_continuous(breaks = seq(0, 2, 0.001))
```
...
ggplot(aes(pH), 
       data = subset(data, !(pH %in% outliers))) +
...
ggplot(aes(sulphates), 
       data = subset(data, !(sulphates %in% outliers))) +
  geom_histogram(bins = 35, fill = 'darkred') +
  scale_x_continuous(breaks = seq(0, 20, 0.05))
```  
...
```{r echo = FALSE}
outliers <- boxplot.stats(data$alcohol)$out

ggplot(aes(alcohol), 
       data = subset(data, !(alcohol %in% outliers))) +
  geom_histogram(bins = 35, fill = 'darkred') +
  scale_x_continuous(breaks = seq(0, 20, 0.5))
```  
...
```{r echo = FALSE}
ggplot(aes(quality.bins), data = data) +
...
means$total.sulfur.dioxide <- rescale(means$total.sulfur.dioxide,
                                      to = c(0.5,1))
...
ggRadar(data = means, aes(color = quality.bins), rescale = FALSE, 
        size = 1.5, ylim = c(0, 1))
```
***
...
p3 <- ggplot(data = data, aes(x = quality, y = density, 
                              fill = quality.bins)) +
  geom_jitter(alpha = 0.1) +
  geom_boxplot(alpha = 0.7) +
  guides(fill=FALSE) +
...
p4 <- ggplot(data = data, aes(x = quality, y = chlorides, 
                              fill = quality.bins)) +
  geom_jitter(alpha = 0.1) +
  geom_boxplot(alpha = 0.7) +
  guides(fill=FALSE) +
...
p2 <- ggplot(data = data, aes(x = quality, y = chlorides, 
                              fill = quality.bins)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(alpha = 0.7) +
  guides(fill=FALSE) +
...
p2 <- ggplot(data = data, aes(x = quality, y = residual.sugar, 
                              group = quality, 
                              fill = quality.bins)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(alpha = 0.7) +
...
p1 <- ggplot(data = data, aes(x = fixed.acidity, y = citric.acid)) + 
  ...
  scale_y_continuous(trans = 'identity') +
  geom_smooth(method = 'loess')
p2 <- ggplot(data = data, aes(x = fixed.acidity, y = pH, 
                              color = citric.acid)) + 
...
```{r echo=FALSE, warning=FALSE}
ggplot(data = data, aes(x = alcohol, y = volatile.acidity, color = factor(quality))) + 
  geom_jitter(alpha = 0.7) +
  scale_color_brewer(palette = 'Spectral', direction = 0) +
  scale_y_continuous(trans = 'identity', breaks = seq(0, 2, 0.1)) +
  scale_x_continuous(breaks = seq(0, 20, 0.5))
```
...
```{r echo=FALSE}
ggplot(data = subset(data, quality.bins != 'average'), 
       aes(x = alcohol, 
           y = volatile.acidity, 
           color = quality.bins)) + 
  geom_jitter(alpha = 0.7) +
...
```{r echo=FALSE, warning=FALSE}
cv.lm(data = data, m7, m=2, printit = FALSE)
```
...
svm.model <- svm(formula, data = train, kernel='radial', 
...
ggplot(data = subset(data, quality.bins != 'average'), 
  ...
  scale_y_continuous(breaks = seq(0, 2, 0.1)) +
  scale_x_continuous(breaks = seq(0, 14, 0.5)) +
  labs(title = 'Profile of the Best and Worst Wines', 
